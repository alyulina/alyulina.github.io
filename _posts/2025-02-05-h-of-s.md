...
from where we recover the familiar allele frequency distribution,  

$$p(f) \sim \frac{N\mu}{f}.$$

{% comment %}
This calculation emphasizes that is $$p(f)$$ evenly distributed on a logarithmic scale, with equal contributions from all orders of magnitude in $$f$$.
{% endcomment %}
Note that this calculation is only valid for $$t \ll f/N$$. At longer times, there is $$\sim\mathcal{O}(1)$$ probability that a mutant lineage will have reached $$f\sim 1$$, or _fixed_. For a steady state to exist we thus need an influx of new mutations; [Ewens (1963)](https://www.jstor.org/stable/2984310?seq=1) showed that this can be achieved by restarting the process at some infinitesimal frequency $$\epsilon$$ immediately upon fixation<sup>4</sup>.
...
{% comment %}
TODO: add discussion / implications.
{% endcomment %}
...
## Deleterious alleles
The frequency dynamics of alleles with a fitness cost $$s$$ are different only in that selection prevents them from reaching frequencies much larger than $$1/Ns$$. [TODO make a note on the distribution] We can see that moments of $$p(f)$$ such as $$H$$ will be dominated by these frequencies, provided $$Ns \gg 1$$,  

$$ H \sim \langle f(1-f) \rangle \sim \int_{\epsilon}^{1/Ns} f(1-f) \cdot p(f) df \sim \frac{\mu}{s}. $$  

Thus, strong negative selection lowers heterozygosity.
...
Similarly, trajectories of alleles with a fitness benefit $$s$$ will be governed by drift at low enough frequencies. With probability $$s$$, a beneficial allele will drift to frequency $$f\sim 1/Ns$$ – or _establish_ – and then expand deterministically – or _sweep_ – due to selection as $$\partial_t f = sf(1-f)$$. Averaging the logistic growth function over the origination times of the mutation, we obtain $$p(f) \sim 1/f(1-f)$$.


are different only in that selection prevents them from reaching frequencies much larger than $$1/Ns$$. We can see that moments of $$p(f)$$ such as $$H$$ will be dominated by these frequencies, provided $$Ns \gg 1$$,  
...
---  
<sup>1</sup> In diploid organisms, heterozygosity often refers to the probability that two alleles at a locus differ in state _within an individual_, rather than across the population. If the dominance coefficient is $$h=1/2$$, the diploid system is equivalent to the haploid case considered here, with a rescaled population size. Other kinds of dominance fall outside our simple model.  
...
<sup>4</sup> Specifically, reinstating the mutant lineage in this way only changes the overall normalization constant of the distribution $$p(f)$$, without altering its shape.   
